...
up front, and the install layouts armillary looks at on each operating system.
...
files, etc.) is fine — armillary only cares that `plugin.toml` exists at the
...
[`discovery.md`](./discovery.md) for how armillary finds these directories on
each OS.
...
`armillary plugin path` prints the directories armillary will scan, in
...
| Linux   | `$XDG_DATA_HOME/armillary/plugins` (default `~/.local/share/armillary/plugins`) |
...
directory into `./plugins/` next to where you run armillary (the highest-priority
scan root), or by setting `ARMILLARY_PLUGIN_PATH` to a directory of your
choice. See [`discovery.md`](./discovery.md) §Scan order for the full
precedence rules.

## Cross-platform binaries

Armillary runs on macOS (Intel + Apple Silicon), Linux (x86_64 + aarch64), and
Windows (x86_64). Your plugin should ship binaries for the platforms you
want to support — armillary will not cross-compile for users.
...
- **Binary suffix on Windows.** The `executable = "my-plugin"` field in
  ...
  `my-plugin` name and let armillary find `my-plugin.exe` automatically — armillary
...
The hard case is plugins that need a runtime (Node.js, Python, JVM). Armillary
...
`examples/plugins/parquet-plugin/` builds a single `armillary-parquet-plugin`
binary and that is the entire deliverable.

### Pattern 2: bundled runtime (Node.js, Python)

The OpenBoard plugin (`openboard/plugins/armillary/`) is the worked example
here. Its `scripts/bundle.mjs` produces a self-contained `dist/plugin/`
directory that armillary can launch as-is:
...
2. **What it requires.** OS support, runtime prerequisites (Node 18+,
   Python 3.10+, libpq, etc.), the minimum armillary version (`armillary_min_version`
   in your manifest).
3. **How to install.** A literal copy-paste block using `armillary
   plugin path` like the one above. Include the `chmod +x` step on Unix
   if you ship a `.zip`.
4. **How to verify.** `armillary plugin check <name>` and what success
   looks like.
...
and your binary's `--version` output if you have one. Armillary surfaces this
in `armillary plugin list` and the canvas Plugins panel, so users can
tell at a glance which version is installed.

The `armillary_plugin_protocol` field is the wire protocol version your plugin
