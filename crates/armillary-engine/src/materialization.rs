...
pub enum HardDeletes {
    ...
    #[default]
    Ignore,
    /// Close the current version with `armillary_valid_to = now()`.
    Invalidate,
...
/// The watermark column armillary uses to determine "new since last run."
...
/// pull in `chrono`/`iso8601` here because armillary-engine deliberately stays
