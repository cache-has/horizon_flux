...
//!
//! On first use, creates a Python virtual environment at `~/.horizon-flux/python/`
...
//!
//! The environment location can be overridden via `HORIZON_FLUX_PYTHON_ENV` for
...
///
/// Priority:
/// 1. `HORIZON_FLUX_PYTHON_ENV` env var (override for testing/custom deployments)
/// 2. `~/.horizon-flux/python/`
pub fn managed_env_dir() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("HORIZON_FLUX_PYTHON_ENV") {
        return Some(PathBuf::from(p));
    }
    home_dir().map(|h| h.join(".horizon-flux").join("python"))
}
...
/// Find the `uv` binary.
///
/// Checks `HORIZON_FLUX_UV` env var first, then falls back to `uv` on PATH.
pub fn find_uv() -> Result<String, NodeErrorKind> {
    if let Ok(uv) = std::env::var("HORIZON_FLUX_UV") {
        return Ok(uv);
    }
    ...
    match Command::new("uv").arg("--version").output() {
        ...
        _ => Err(NodeErrorKind::Python(
            "`uv` is not installed. Install it from https://docs.astral.sh/uv/ \
             or set HORIZON_FLUX_UV to the path of the uv binary."
                .to_string(),
        )),
    }
}
...
mod tests {
    ...
    fn managed_env_dir_uses_override() {
        ...
        // HORIZON_FLUX_PYTHON_ENV concurrently.
        unsafe {
            std::env::set_var("HORIZON_FLUX_PYTHON_ENV", &path);
        }
        let dir = managed_env_dir().unwrap();
        unsafe {
            std::env::remove_var("HORIZON_FLUX_PYTHON_ENV");
        }
