...
//! `flux plugin` subcommands — discovery, inspection, and smoke-testing of
//! installed plugins. See `planning/24-plugin-system.md` § flux-cli.
...
pub enum PluginAction {
    ...
    /// Print the directories flux scans for plugins, in priority order.
    Path,
}
    ...
    match format {
        OutputFormat::Human => {
            ...
            if let Some(m) = &plugin.manifest {
                ...
                println!("protocol:  {}", m.flux_plugin_protocol);
                println!("min flux:  {}", m.flux_min_version);
