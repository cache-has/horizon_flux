...
use armillary_engine::node::{NodeKind, SourceConfig, TransformMode};
use armillary_engine::variables::{BuiltinContext, ResolvedVariables};
use armillary_engine::{NodeId, Pipeline, dag};
use armillary_observability::emit_event;
use armillary_observability::events as obs;
use armillary_observability::metrics as prom;
use armillary_observability::openlineage as ol;
...
/// `armillary-secrets` crate directly. The server provides an implementation backed
...
pub trait SecretResolver: Send + Sync {
    ...
    /// [`armillary_secrets::scrub_secrets`] to redact secrets from error messages.
    ///
...
pub struct ExecutionOptions {
    ...
    /// Used by `armillary snapshot diff` (doc 28) to drive a true dry-run.
    ...
    pub fingerprint_fn: Option<armillary_engine::FingerprintFn>,
...
impl PipelineExecutor {
    ...
    pub async fn execute(
        ...
        // Skipped under `dry_run_no_sinks` (doc 28 `armillary snapshot diff`):
        ...
        let incremental_plans = if options.dry_run_no_sinks {
            ...
            .map_err(|e| {
                ...
                let sink_id = pipeline
                    .nodes
                    .iter()
                    .find(|n| matches!(n.kind, armillary_engine::node::NodeKind::Sink(_)))
                    .map(|n| n.id.clone())
        ...
        for node_id in &order {
            ...
            let result: Result<Vec<RecordBatch>, NodeErrorKind> = match &node.kind {
                ...
                NodeKind::Sink(sink_cfg) => {
                    ...
                    // `armillary snapshot diff`) can read them; we just don't
            ...
            match result {
                        ...
                        let obs = match &node.kind {
                            NodeKind::Source(src) => {
                                fp_fn(&src.connector, &src.config).map(|fp| LineageObservation {
                                    ...
                                    direction: armillary_engine::BindingDirection::Source,
                            ...
                            NodeKind::Sink(sink) => {
                                fp_fn(&sink.connector, &sink.config).map(|fp| LineageObservation {
                                    ...
                                    node_id: node_id.0.clone(),
                                    run_id: run.id.0.to_string(),
                                    direction: armillary_engine::BindingDirection::Sink,
                                    resource_fingerprint: fp,
                                    environment: options.environment.clone(),
                                    observed_at_ms: node_end_wall
                                        .duration_since(std::time::UNIX_EPOCH)
                ...
                Err(kind) => {
                    ...
                    // or API responses.
                    let error_msg =
                        armillary_secrets::scrub_secrets(&kind.to_string(), &node_secret_values);
                    ...
                    if let Some(store) = &options.run_store {
                        ...
                        let scrubbed_chain: Vec<String> = error_chain
                            .iter()
                            .map(|msg| armillary_secrets::scrub_secrets(msg, &node_secret_values))
                            .collect();
                        let input_schemas =
    ...
    pub async fn execute_sql_transform_with_lineage(
        ...
        (
            Vec<RecordBatch>,
            Option<armillary_engine::NodeColumnLineage>,
        ),
        NodeErrorKind,
    > {
        let ctx = match session_factory {
    ...
    async fn execute_sink(
        node_id: &NodeId,
        config: &armillary_engine::node::SinkConfig,
...
fn watermark_type_str(t: armillary_engine::materialization::WatermarkType) -> &'static str {
    use armillary_engine::materialization::WatermarkType;
    match t {
...
fn persist_column_lineage(
    ...
    pipeline_id: &str,
    environment: &str,
    lineage: &armillary_engine::NodeColumnLineage,
    run_id: Option<&RunId>,
...
fn collect_ol_inputs(
    ...
    fingerprint_fn: Option<&armillary_engine::FingerprintFn>,
) -> Vec<ol::InputDataset> {
