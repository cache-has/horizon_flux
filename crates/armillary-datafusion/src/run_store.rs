...
use armillary_engine::column_lineage::{ColumnEdge, Confidence, RelationshipKind};
...
impl SqliteRunStore {
    ...
    ///
    /// Used by `armillary metadata import` to copy data from a remote store.
...
impl crate::storage::SlaStorage for SqliteRunStore {
    ...
    fn latest_evaluations(&self) -> Result<Vec<armillary_engine::SlaEvaluation>, RunStoreError> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
    ...
    fn latest_evaluation(
        &self,
        fingerprint: &str,
    ) -> Result<Option<armillary_engine::SlaEvaluation>, RunStoreError> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT fingerprint, evaluated_at, status, age, max_age, warn_at,
                    producer_pipeline, last_success_at
    ...
    fn evaluation_history(
        ...
        fingerprint: &str,
        limit: u32,
    ) -> Result<Vec<armillary_engine::SlaEvaluation>, RunStoreError> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT fingerprint, evaluated_at, status, age, max_age, warn_at,
                    producer_pipeline, last_success_at
...
fn row_to_sla_evaluation(
    row: &rusqlite::Row<'_>,
) -> Result<armillary_engine::SlaEvaluation, RunStoreError> {
    ...
    Ok(armillary_engine::SlaEvaluation {
...
mod tests {
    ...
    fn sla_evaluations_roundtrip() {
        ...
        let latest = store
            ...
            .unwrap()
            .unwrap();
        assert_eq!(latest.status, armillary_engine::SlaStatus::Warning);
