...
/// - `ARMILLARY_PYTHON_TIMEOUT`: max seconds (default 300 = 5 minutes)
/// - `ARMILLARY_PYTHON_MEMORY_LIMIT`: max RSS in bytes (default: none)
#[derive(Debug, Clone)]
...
impl Default for PythonConfig {
    fn default() -> Self {
        let timeout = std::env::var("ARMILLARY_PYTHON_TIMEOUT")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
            .map(Duration::from_secs)
            .unwrap_or(Duration::from_secs(300));

        let memory_limit = std::env::var("ARMILLARY_PYTHON_MEMORY_LIMIT")
            .ok()
...
///
/// Search order:
/// 1. `ARMILLARY_PYTHON` env var (explicit override)
/// 2. `VIRTUAL_ENV/bin/python3` (active venv)
/// 3. `.venv/bin/python3` relative to the workspace root (project-local venv)
/// 4. `~/.armillary/python/bin/python3` (managed env created by `ensure_python_env`)
/// 5. `python3` on PATH
fn find_python() -> String {
    // 1. Explicit override.
    if let Ok(p) = std::env::var("ARMILLARY_PYTHON") {
        return p;
    }
    ...
    // 4. Managed environment (~/.armillary/python/).
