...
use armillary_datafusion::backfill::{BackfillEvent, BackfillRunOptions};
...
impl PipelineSink for MockSink {
    async fn write(
        ...
        ctx: &armillary_datafusion::provider::MaterializationContext,
    ) -> Result<armillary_datafusion::provider::MaterializationReceipt, ProviderError> {
        ...
        Ok(armillary_datafusion::provider::MaterializationReceipt::from_write_stats(&stats, ctx))
    }
...
impl PipelineSink for FailingSink {
    async fn write(
        ...
        _options: &WriteOptions,
        _ctx: &armillary_datafusion::provider::MaterializationContext,
    ) -> Result<armillary_datafusion::provider::MaterializationReceipt, ProviderError> {
...
async fn coordinator_respects_concurrency_limit() {
    ...
    let (_, progress) = armillary_datafusion::backfill::start_backfill(backfill, opts)
        .await
        .unwrap();
...
async fn resume_skips_succeeded_iterations() {
    ...
    let (_, progress) = armillary_datafusion::backfill::resume_backfill(&bf_id, opts)
        .await
        .unwrap();
...
async fn cancellation_stops_new_iterations() {
    ...
    let (_, _progress) = armillary_datafusion::backfill::start_backfill(backfill, opts)
        .await
        .unwrap();
