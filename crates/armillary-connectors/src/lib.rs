...
//! Concrete source and sink connector implementations for Armillary.
//!
//! This crate provides:
...
pub fn default_registry() -> ConnectorRegistry {
    ...
    let file_source: Arc<dyn armillary_datafusion::provider::SourceConnector> =
    ...
    let pg_source: Arc<dyn armillary_datafusion::provider::SourceConnector> =
    ...
    let rest_source: Arc<dyn armillary_datafusion::provider::SourceConnector> =
    ...
    let file_sink: Arc<dyn armillary_datafusion::provider::PipelineSink> =
    ...
    let pg_sink: Arc<dyn armillary_datafusion::provider::PipelineSink> =
        Arc::new(PostgresSink::new());
    registry.register_sink("postgresql", Arc::clone(&pg_sink));
    registry.register_sink("postgres", pg_sink);

    let stdout_sink: Arc<dyn armillary_datafusion::provider::PipelineSink> =
...
/// supplied [`armillary_plugin_host::PluginRegistry`].
///
...
pub fn default_registry_with_plugins(
    plugins: Arc<armillary_plugin_host::PluginRegistry>,
) -> ConnectorRegistry {
    let mut registry = default_registry();
    let plugin_sink: Arc<dyn armillary_datafusion::provider::PipelineSink> =
        Arc::new(PluginSink::new(Arc::clone(&plugins)));

    for plugin in plugins.iter() {
        let manifest = match (&plugin.status, &plugin.manifest) {
            (armillary_plugin_host::PluginStatus::Ok, Some(m)) => m,
            _ => continue,
        };
