...
use armillary_datafusion::provider::{
    PipelineSink, ProviderError, ProviderRegistry, SourceConnector,
};
...
impl ConnectorRegistry {
    ...
    pub fn validate_pipeline(
        &self,
        pipeline: &armillary_engine::Pipeline,
    ) -> Result<(), Vec<String>> {
        use std::collections::HashMap;
        ...
        for node in &pipeline.nodes {
            match &node.kind {
                armillary_engine::node::NodeKind::Source(cfg) => {
                ...
                armillary_engine::node::NodeKind::Sink(cfg) => {
                    node_connectors.insert(node.id.0.as_str(), cfg.connector.as_str());
                ...
                armillary_engine::node::NodeKind::Transform(_)
                | armillary_engine::node::NodeKind::Test(_) => {}
                armillary_engine::node::NodeKind::Snippet(_) => {
        ...
        for (env_name, env_overrides) in &pipeline.environment_overrides {
            for (node_id, override_val) in env_overrides {
                let Some(connector_type) = node_connectors.get(node_id.as_str()) else {
                    ...
                    // checked by armillary-engine's validate_import, and transforms
                    // don't have connector configs to override.
                    if pipeline.nodes.iter().any(|n| {
                        n.id.0 == *node_id
                            && matches!(n.kind, armillary_engine::node::NodeKind::Transform(_))
                    }) {
                        errors.push(format!(
