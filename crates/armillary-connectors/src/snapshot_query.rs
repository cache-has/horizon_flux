...
//!
//! Lives in `armillary-connectors` rather than `armillary-cli` so both the CLI
//! (`armillary snapshot history`) and the HTTP server (snapshot history viewer
//! in the sink editor — planning doc 28) can call into the same code path
//! without armillary-server having to depend on armillary-cli (which would create a
//! cycle: armillary-cli already depends on armillary-server).
//!
...
pub enum SnapshotQueryError {
    ...
    #[error(
        "`armillary snapshot` v1 supports the `postgresql` sink only — node `{node_id}` uses connector \
...
pub fn resolve_snapshot_sink(
    ...
    // Mirror armillary-cli/src/snapshot.rs: env override → var interpolation →
...
//
// Shared between `armillary snapshot diff` (CLI) and the `snapshot/diff` HTTP
...
/// against a snapshot target's `armillary_is_current = true` slice.
...
/// where `armillary_is_current = true`. The signal is computed inside Postgres
