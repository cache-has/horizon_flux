...
//! `armillary_engine::validate::migrate_legacy_sinks`.
...
fn pg_write_from_policy(policy: Option<&MaterializationPolicy>) -> Result<PgWrite, ProviderError> {
    ...
    match p.write_strategy {
        ...
        WriteStrategy::Merge => {
            ...
            if keys.is_empty() {
                // validate_policy in armillary-engine should already reject this,
...
impl PipelineSink for PostgresSink {
    async fn write(
        ...
        // Snapshot path also creates a helper index on (unique_keys, armillary_is_current)
        ...
        } = &write_mode
        {
            ...
            if matches!(policy.hard_deletes, HardDeletes::Delete) && !gone_ids.is_empty() {
                ...
                // their armillary_scd_id above; for DELETE we need the business
                ...
                let select_sql = format!(
                    "SELECT DISTINCT {} FROM {} WHERE \"armillary_scd_id\" = ANY($1)",
                    key_cols_select,
                    quote_ident(table)
                );
