...
//!
//! `armillary_engine::snapshot` (the pure planner) is still the canonical
...
//! [`armillary_engine::SnapshotMergeStats`] roll-up so the
//! `MaterializationReceipt` accounting from doc 28 is consistent
//! sink-to-sink.

use armillary_datafusion::provider::ProviderError;
use armillary_engine::materialization::{ChangeDetection, HardDeletes, SnapshotPolicy};
...
/// `armillary_engine::materialization::validate_policy`; this is the runtime
...
/// `armillary_engine::snapshot::scd_metadata_columns`.
///
...
pub(crate) fn build_create_snapshot_table_sql(
    ...
    // Index on the business key + armillary_is_current accelerates every
...
/// the standard pattern: `(unique_keys, armillary_is_current)` so the
...
///
/// - `changed_ids text[]` — `armillary_scd_id` of every current target row
///   whose business key matches a stage row but whose comparison columns
///   differ. These get closed and a new version inserted.
/// - `gone_ids text[]` — `armillary_scd_id` of every current target row whose
...
/// `UPDATE target SET armillary_valid_to = $1, armillary_is_current = false WHERE armillary_scd_id = ANY($2)`.
...
/// Postgres-side md5 (rather than `armillary_engine::snapshot::surrogate_key`)
/// so the value is generated atomically with the INSERT and we don't have
/// to bind one parameter per row. The armillary-engine helper is still
...
mod tests {
    ...
    fn create_snapshot_table_sql_has_metadata_at_end() {
        ...
        let scd_id_at = sql.find("\"armillary_scd_id\"").unwrap();
        let valid_from_at = sql.find("\"armillary_valid_from\"").unwrap();
        let valid_to_at = sql.find("\"armillary_valid_to\"").unwrap();
        let is_current_at = sql.find("\"armillary_is_current\"").unwrap();
        ...
        assert!(sql.contains("\"armillary_valid_to\" TIMESTAMPTZ,"));
        assert!(sql.contains("\"armillary_scd_id\" TEXT NOT NULL"));
        assert!(sql.contains("\"armillary_is_current\" BOOLEAN NOT NULL"));
    }
    ...
    fn diff_query_includes_all_three_buckets() {
        ...
        assert!(sql.contains("\"armillary_is_current\""));
    }
    ...
    fn close_versions_sql_uses_array_param() {
        ...
        assert!(sql.contains("\"armillary_valid_to\" = $1"));
        assert!(sql.contains("\"armillary_is_current\" = false"));
    }
    ...
    fn insert_new_versions_sql_uses_md5_surrogate_and_correct_filter() {
        ...
        assert!(sql.contains("\"armillary_scd_id\""));
        assert!(sql.contains("\"armillary_valid_from\""));
