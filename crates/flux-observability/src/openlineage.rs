...
//! ABORT events so flux pipelines are discoverable in data catalogs like
...
///
/// Flux's internal lineage model is richer (relationship kinds, confidence
...
/// Convert a flux run ID to a UUID string.
///
/// Flux run IDs are UUIDs but stored as plain strings. If the string is
