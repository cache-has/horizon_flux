...
use flux_observability::config::{LogFormat, LogLevel, LoggingConfig};
use flux_observability::events::*;
...
fn setup_json_subscriber() -> (BufWriter, tracing::subscriber::DefaultGuard) {
    ...
    let layer = flux_observability::json_format::FluxJsonLayer::new(buf.clone());
...
fn structured_flux_event_produces_documented_envelope() {
    ...
    flux_observability::emit_event!(event);
