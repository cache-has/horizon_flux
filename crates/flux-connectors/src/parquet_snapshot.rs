...
//! [`flux_engine::snapshot`], and writes a new Parquet file containing the
            ...
            let is_current_idx = target.schema().index_of(FLUX_IS_CURRENT).map_err(|_| {
                format!(
                    "existing target '{}' has no `{FLUX_IS_CURRENT}` column; \
                         it does not look like a flux snapshot table",
                    path.display()
                )
            })?;
...
/// change-detection comparison value defined by [`flux_engine::snapshot`].
...
/// Take a slice of current target rows and overwrite their `flux_valid_to`
/// and `flux_is_current` to mark them closed at `now_micros`. Other columns
/// (including `flux_scd_id` and the original `flux_valid_from`) are preserved.
