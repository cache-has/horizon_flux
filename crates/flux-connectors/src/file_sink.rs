...
use flux_engine::materialization::WriteStrategy;
    ...
    match (format, strategy) {
        ...
        (FileFormat::Csv, WriteStrategy::Snapshot) => Err(
            "csv file sink cannot represent SCD2 history (no schema for \
             flux_valid_from/flux_valid_to/flux_is_current/flux_scd_id metadata); \
...
async fn replace_cloud_partition(
    ...
    // version of flux ever wrote nested files under a partition, they'll
...
mod tests {
    ...
    async fn parquet_truncate_insert_overwrites_even_with_append_mode() {
        ...
        use flux_datafusion::provider::MaterializationContext;
        use flux_engine::materialization::{MaterializationPolicy, ReadMode};
        use std::sync::Arc;
