...
) -> flux_datafusion::provider::MaterializationReceipt {
...
async fn parquet_snapshot_lifecycle_full_check() {
    ...
    // All flux_is_current = true, all flux_valid_to = NULL.
    let is_cur = col_bool(&t1, "flux_is_current");
    let vt = col_ts(&t1, "flux_valid_to");
    ...
    let ids: std::collections::HashSet<&str> = (0..t1.num_rows())
        .map(|i| col_str(&t1, "flux_scd_id").value(i))
        .collect();
    assert_eq!(ids.len(), 3);
    ...
    let cur_count = (0..t2.num_rows())
        .filter(|i| col_bool(&t2, "flux_is_current").value(*i))
        .count();
    ...
    for i in 0..t3.num_rows() {
        ...
        if col_bool(&t3, "flux_is_current").value(i) {
            current_for_2 += 1;
            assert_eq!(col_str(&t3, "plan").value(i), "enterprise");
        } else {
            closed_for_2 += 1;
            assert!(!col_ts(&t3, "flux_valid_to").is_null(i));
    ...
    let three_current = (0..t4.num_rows())
        .filter(|i| {
            col_i64(&t4, "customer_id").value(*i) == 3 && col_bool(&t4, "flux_is_current").value(*i)
        })
        .count();
    assert_eq!(
        three_current, 0,
