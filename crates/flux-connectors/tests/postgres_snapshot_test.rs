...
//! `HORIZON_FLUX_TEST_PG_URL` to a connection string, or they default to
...
//! in `crates/flux-postgres/tests/postgres_test.rs`.
//!
...
use flux_connectors::PostgresSink;
...
/// trick `crates/flux-postgres/tests/postgres_test.rs` uses.
...
async fn current_count(client: &Client, table: &str) -> i64 {
    let row = client
        .query_one(
            &format!("SELECT COUNT(*) FROM \"{table}\" WHERE flux_is_current"),
            &[],
        )
        .await
...
async fn snapshot_first_run_inserts_all_as_current_versions() {
    ...
    let row = client
        .query_one(
            &format!("SELECT COUNT(DISTINCT flux_scd_id) FROM \"{table}\""),
            &[],
        )
        .await
        .unwrap();
...
async fn snapshot_second_run_unchanged_is_idempotent() {
    ...
    let before: Vec<String> = client
        .query(
            &format!("SELECT flux_scd_id FROM \"{table}\" ORDER BY id"),
            &[],
        )
        .await
        .unwrap()
...
async fn snapshot_changed_row_closes_old_and_opens_new() {
    ...
    let rows = client
        .query(
            &format!(
                "SELECT email, flux_is_current, flux_valid_to IS NULL AS open_ended \
                 FROM \"{table}\" WHERE id = 2 ORDER BY flux_valid_from"
            ),
            &[],
        )
        .await
    ...
    assert!(!old_current);
    assert!(!old_open, "old version's flux_valid_to must be set");
        ...
        .query_one(
            &format!(
                "SELECT email FROM \"{table}\" \
                 WHERE id = 2 \
                   AND now() >= flux_valid_from \
                   AND (flux_valid_to IS NULL OR now() < flux_valid_to)"
            ),
            &[],
        )
        .await
...
async fn snapshot_hard_deletes_invalidate_closes_missing_rows() {
    ...
    let row = client
        .query_one(
            &format!(
                "SELECT flux_is_current, flux_valid_to IS NOT NULL \
...
async fn snapshot_hard_deletes_ignore_keeps_missing_row_current() {
    ...
    let row = client
        .query_one(
            &format!(
                "SELECT flux_is_current, flux_valid_to IS NULL \
                 FROM \"{table}\" WHERE id = 3"
            ),
            &[],
        )
