...
//! Rust SDK for writing Armillary sink plugins.
//!
...
use armillary_plugin_protocol::arrow_ipc::{ArrowIpcError, decode_record_batch, decode_schema_b64};
...
pub enum RunError {
    ...
    #[error("frame error: {0}")]
    Frame(#[from] armillary_plugin_protocol::FrameError),
...
pub fn run<S: Sink>(info: PluginInfo, sink: S) -> ExitCode {
    ...
    match run_io(info, sink, &mut stdin, &mut stdout) {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("armillary-plugin-sdk: fatal: {e}");
            ExitCode::FAILURE
        }
    }
}
...
/// };
/// use armillary_plugin_sdk::{PluginInfo, Sink, SinkError, WriteStats, run_io};
///
...
///     protocol: PROTOCOL_VERSION, armillary_version: "doctest".into(),
/// }).unwrap();
...
/// let _ = armillary_plugin_protocol::read_frame(&mut out).unwrap();
...
fn drain_until_shutdown<R: Read>(reader: &mut R) -> Result<(), RunError> {
    loop {
        match read_frame(reader) {
            ...
            Ok(_) => continue,
            Err(armillary_plugin_protocol::FrameError::UnexpectedEof { .. })
            | Err(armillary_plugin_protocol::FrameError::Io(_)) => return Ok(()),
