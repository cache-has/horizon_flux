...
use armillary_datafusion::RunStatus;
use armillary_engine::catalog::{self, Catalog};
use armillary_engine::lineage::{LineageGraph, ResourceBinding};
use armillary_engine::pipeline_store::PipelineId;
use armillary_engine::sla::{SlaEvaluation, SlaStatus, format_iso_duration, parse_iso_duration};
use armillary_observability::emit_event;
use armillary_observability::events::{FluxEvent, SlaBreach, SlaWarning};
    ...
    for (fp, ann_file) in &annotations {
        ...
        match status {
            SlaStatus::Breach => {
                ...
                armillary_observability::metrics::record_sla_status(&fp.0, "breach");
            }
            SlaStatus::Warning => {
                ...
                armillary_observability::metrics::record_sla_status(&fp.0, "warning");
            }
            SlaStatus::Ok => {
                armillary_observability::metrics::record_sla_status(&fp.0, "ok");
            }
            SlaStatus::Unknown => {
                armillary_observability::metrics::record_sla_status(&fp.0, "unknown");
            }
        }
