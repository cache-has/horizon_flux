...
use armillary_connectors::ConnectorRegistry;
...
use armillary_engine::PipelineStorage;
use armillary_plugin_host::PluginRegistry;
use armillary_scheduler::{Scheduler, TriggerStorage};
use armillary_secrets::SecretStore;
...
impl SecretSession {
    ...
    pub fn unlock(&mut self, password: &[u8]) -> Result<(), armillary_secrets::SecretError> {
    ...
    pub fn init(&mut self, password: &[u8]) -> Result<(), armillary_secrets::SecretError> {
...
pub struct MetadataInfo {
    ...
    /// How the backend was configured: `"--metadata-url flag"`, `"ARMILLARY_METADATA_URL env var"`,
...
pub fn redact_connection_string(conn: &str) -> String {
    ...
    // Key-value style: host=localhost password=secret dbname=armillary
...
pub struct AppState {
    ...
    pub openlineage_client: Option<Arc<armillary_observability::openlineage::OpenLineageClient>>,
...
/// store is unlocked, and delegates to [`armillary_secrets::resolve_json_secrets`].
...
impl SecretResolver for SessionSecretResolver {
    ...
    fn resolve_json_collecting(
        ...
        armillary_secrets::resolve_json_secrets_collecting(value, store, environment)
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)
    }
}
