...
//!   version history (sibling to `armillary snapshot history`).
//! - `POST /api/pipelines/{id}/nodes/{node_id}/snapshot/diff` — dry-run
//!   diff preview: runs the upstream DAG with `dry_run_no_sinks`, then
//!   classifies the staged batch against the target's `armillary_is_current`
...
use armillary_connectors::snapshot_query::{
...
use armillary_datafusion::{ExecutionOptions, PipelineExecutor};
...
pub struct HistoryVersionDto {
    pub armillary_scd_id: String,
    pub armillary_valid_from: String,
    pub armillary_valid_to: Option<String>,
    pub armillary_is_current: bool,
    ...
    let sample = summary
        ...
        .map(|s| SnapshotDiffSampleDto {
            classification: match s.classification {
                armillary_connectors::snapshot_query::DiffClassification::Unchanged => {
                    "unchanged".into()
                }
                armillary_connectors::snapshot_query::DiffClassification::Changed => {
                    "changed".into()
                }
                armillary_connectors::snapshot_query::DiffClassification::New => "new".into(),
                armillary_connectors::snapshot_query::DiffClassification::Gone => "gone".into(),
            },
