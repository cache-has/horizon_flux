...
fn derive_observed_edges(
    observations: &[armillary_datafusion::LineageObservation],
) -> Vec<LineageEdge> {
    use std::collections::HashMap;

    let mut sinks: HashMap<&ResourceFingerprint, Vec<&armillary_datafusion::LineageObservation>> =
        HashMap::new();
...
fn build_column_graph(
    ...
    let mut by_pipeline: std::collections::HashMap<PipelineId, Vec<armillary_engine::ColumnEdge>> =
    ...
    let pipeline_edges: Vec<(PipelineId, Vec<armillary_engine::ColumnEdge>)> =
        by_pipeline.into_iter().collect();
    let pipeline_edge_refs: Vec<(PipelineId, &[armillary_engine::ColumnEdge])> = pipeline_edges
        .iter()
...
fn trace_edges_to_dtos(edges: &[armillary_engine::column_lineage::TraceEdge]) -> Vec<TraceEdgeDto> {
    edges
        .iter()
        .map(|e| TraceEdgeDto {
