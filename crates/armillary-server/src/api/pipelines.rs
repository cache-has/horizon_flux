...
use armillary_engine::{NodeId, Pipeline, SampleConfig};
use axum::Json;
use axum::Router;
...
fn refresh_lineage_bindings(state: &AppState, pipeline_id: &PipelineId, pipeline: &Pipeline) {
    ...
    for node in &pipeline.nodes {
        ...
        if let Some(fp) = armillary_connectors::fingerprint::fingerprint(connector, config) {
...
impl From<armillary_engine::PipelineRecord> for PipelineResponse {
    fn from(r: armillary_engine::PipelineRecord) -> Self {
...
pub struct PreviewNodeResponse {
    ...
    pub status: armillary_datafusion::PreviewStatus,
}
...
async fn create_pipeline(
    ...
    let record = state.pipeline_store.create(pipeline).map_err(|e| {
        use armillary_engine::PipelineStoreError;
        match &e {
...
async fn update_pipeline(
    ...
    for node in &mut pipeline.nodes {
        if let armillary_engine::node::NodeKind::Transform(ref mut xform) = node.kind {
...
async fn run_pipeline(
    ...
    let override_errors =
        armillary_engine::variables::validate_overrides(&record.pipeline, &req.variables);
...
async fn import_pipeline(
    ...
    let (mut pipeline, import_warnings) =
        Pipeline::from_json_with_warnings(&json_str).map_err(|e| match e {
            armillary_engine::ImportError::Json(je) => {
                ApiError::bad_request(format!("invalid pipeline JSON: {je}"))
            }
            armillary_engine::ImportError::Validation(errors) => ApiError::bad_request(format!(
            ...
            armillary_engine::ImportError::Snippet(se) => {
...
async fn bulk_export(
    ...
    Ok((
        ...
        [
            ...
            (
                axum::http::header::CONTENT_DISPOSITION,
                "attachment; filename=\"armillary-pipelines.json\"".to_string(),
            ),
        ],
        json,
    )
...
async fn get_failure_reproduce_bundle(
    ...
    let bundle = armillary_datafusion::ReproduceBundle::from_failure_report(&report);
    let json =
