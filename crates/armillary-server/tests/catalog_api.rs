...
use armillary_scheduler::SqliteTriggerStore;
...
fn test_router(state: AppState) -> Router {
    Router::new()
        .nest("/api/catalog", armillary_server::api::catalog::router())
        .with_state(state)
}
...
async fn update_metadata_creates_annotation() {
    ...
    match event {
        armillary_server::CatalogEvent::MetadataUpdated { fingerprint } => {
...
async fn auto_derived_freshness_from_run_history() {
    ...
    let pipeline: armillary_engine::Pipeline = serde_json::from_value(json!({
        "name": "etl-orders",
        "nodes": [],
        "edges": []
    }))
    ...
    state
        ...
        .save_node_stats(
            &run_id,
            &NodeRunStats {
                node_id: armillary_engine::NodeId("sink1".to_string()),
                start_time,
                end_time,
                rows_in: 500,
                rows_out: 500,
