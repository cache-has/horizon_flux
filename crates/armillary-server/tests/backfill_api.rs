...
use armillary_datafusion::{SqliteBackfillStore, SqliteEnvironmentStore, SqliteRunStore};
...
fn test_router(state: AppState) -> Router {
    Router::new()
        .nest("/api/backfills", armillary_server::api::backfills::router())
        .with_state(state)
}
...
fn create_test_pipeline(state: &AppState) -> String {
    let pipeline: armillary_engine::Pipeline = serde_json::from_value(serde_json::json!({
        "name": "test-pipe",
        "nodes": [],
        "edges": []
    }))
