...
use armillary_engine::SqlitePipelineStore;
use armillary_engine::lineage::{BindingDirection, ResourceFingerprint};
use armillary_server::AppState;
use axum::Router;
use axum::body::Body;
...
fn test_state() -> AppState {
    ...
    AppState {
        ...
        secret_session: Arc::new(std::sync::Mutex::new(
            armillary_server::state::SecretSession::new(
                std::env::temp_dir().join("unused-secrets.db"),
            ),
        )),
        event_tx: AppState::new_event_channel(),
        plugin_event_tx: AppState::new_plugin_event_channel(),
        output_cache: Arc::new(armillary_datafusion::OutputCache::new(std::env::temp_dir())),
        session_factory: None,
        metadata_info: armillary_server::state::MetadataInfo {
        ...
        plugin_registry: Arc::new(std::sync::RwLock::new(Arc::new(
            armillary_plugin_host::PluginRegistry::default(),
        ))),
        backfill_store: Arc::new(SqliteBackfillStore::open_in_memory().unwrap()),
        trigger_store: Arc::new(armillary_scheduler::SqliteTriggerStore::open_in_memory().unwrap()),
        scheduler: None,
...
fn test_router(state: AppState) -> Router {
    Router::new()
        .nest("/api/lineage", armillary_server::api::lineage::router())
        .with_state(state)
}
...
async fn pipeline_column_lineage_returns_edges() {
    ...
    let app = Router::new()
        .nest("/api/pipelines", armillary_server::api::pipelines::router())
        .with_state(state);
