...
use armillary_plugin_host::discovery::{PLUGIN_PATH_ENV, PluginStatus, discover_plugins};
use armillary_plugin_host::manifest::Manifest;
use armillary_plugin_host::process::{PluginProcess, SpawnOptions};
use armillary_plugin_host::protocol::{FrameError, MAX_PAYLOAD_LEN, MessageKind, read_frame};
use armillary_plugin_host::session::{PluginSession, SessionError};
use armillary_plugin_host::transport::TransportError;
...
fn discovery_honors_env_var_override_cross_platform() {
    // Cross-platform check: a directory advertised via ARMILLARY_PLUGIN_PATH
