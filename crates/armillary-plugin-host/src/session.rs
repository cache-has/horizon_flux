...
impl<T: Transport> PluginSession<T> {
    pub fn new(transport: T, host_protocol: u32, armillary_version: impl Into<String>) -> Self {
        Self {
            transport,
            state: State::Spawned,
            host_protocol,
            armillary_version: armillary_version.into(),
            plugin_info: None,
    ...
    pub fn handshake_with_timeout(&mut self, timeout: Duration) -> Result<&HelloAck, SessionError> {
        ...
        let hello = Hello {
            ...
            armillary_version: self.armillary_version.clone(),
        };
    ...
    pub fn send_batch(&mut self, batch: &RecordBatch) -> Result<BatchAck, SessionError> {
        ...
        if let Some(w) = &ack.warning {
            warn!(target: "armillary::plugin", warning = %w, "plugin batch warning");
        }
        Ok(ack)
    }
    ...
    pub fn abort(&mut self, reason: impl Into<String>) -> Result<(), SessionError> {
        ...
        match self.recv(timeouts::ABORT_ACK, "abort") {
            ...
            Ok(other) => warn!(
                target: "armillary::plugin",
                kind = ?other.kind,
                "unexpected frame while waiting for AbortAck"
            ),
            Err(e) => warn!(target: "armillary::plugin", error = %e, "abort wait failed"),
        }
