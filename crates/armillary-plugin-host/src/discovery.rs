...
//! 2. Legacy `~/.armillary/plugins` if it exists.
//! 3. `ARMILLARY_PLUGIN_PATH` env var (platform-separator delimited).
...
pub const PLUGIN_PATH_ENV: &str = "ARMILLARY_PLUGIN_PATH";
...
/// armillary process's working directory.
...
/// home dir, `ARMILLARY_PLUGIN_PATH`, and the workspace `./plugins/`).
///
...
/// armillary instance shipping its own plugin set).
...
/// Resolve the ordered list of scan roots. Public so `armillary plugin path` can
/// print it.
pub fn scan_roots(cwd: &Path) -> Vec<PathBuf> {
    let mut roots: Vec<PathBuf> = Vec::new();

    if let Some(pd) = ProjectDirs::from("com", "horizon-analytic", "armillary") {
        roots.push(pd.data_dir().join("plugins"));
    }

    if let Some(home) = dirs_home() {
        let legacy = home.join(".armillary").join("plugins");
        if legacy.exists() {
            roots.push(legacy);
        }
    }
...
"#
    ...
    fn sink_type_collision_marks_second_invalid() {
        ...
        // `ARMILLARY_PLUGIN_PATH`, etc.). `discover_plugins` is exercised
