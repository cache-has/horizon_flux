...
impl PluginProcess {
    ...
    pub fn spawn_with_manifest(
        ...
        if let Some(traceparent) = armillary_observability::otel::current_traceparent() {
...
fn forward_log(plugin: &str, log: Log) {
    match log.level {
        LogLevel::Trace => trace!(target: "armillary::plugin", plugin = %plugin, "{}", log.message),
        LogLevel::Debug => debug!(target: "armillary::plugin", plugin = %plugin, "{}", log.message),
        LogLevel::Info => info!(target: "armillary::plugin", plugin = %plugin, "{}", log.message),
        LogLevel::Warn => warn!(target: "armillary::plugin", plugin = %plugin, "{}", log.message),
        LogLevel::Error => error!(target: "armillary::plugin", plugin = %plugin, "{}", log.message),
    }
}
...
fn stderr_loop(stderr: std::process::ChildStderr, plugin_name: String) {
    ...
    for line in r.lines().map_while(std::result::Result::ok) {
        if !line.is_empty() {
            warn!(target: "armillary::plugin::stderr", plugin = %plugin_name, "{}", line);
        }
    }
}
