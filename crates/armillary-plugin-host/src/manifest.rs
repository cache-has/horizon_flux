...
//! - `version` and `armillary_min_version` must be SemVer 2.0.
//! - `armillary_plugin_protocol` must equal [`crate::PROTOCOL_VERSION`].
...
pub struct Manifest {
    ...
    pub armillary_plugin_protocol: u32,
    pub armillary_min_version: String,
...
pub struct MaterializationCapabilities {
    ...
    /// implements stage-diff-merge with `armillary_valid_from`/`armillary_valid_to`/
    /// `armillary_is_current`/`armillary_scd_id` metadata columns and accepts the
...
impl Manifest {
    ...
    fn validate(&self, source_path: &Path) -> Result<()> {
        ...
        Version::parse(&self.armillary_min_version).map_err(|e| {
            bad(format!(
                "armillary_min_version `{}` is not SemVer: {e}",
                self.armillary_min_version
            ))
        })?;

        if self.armillary_plugin_protocol != PROTOCOL_VERSION {
            return Err(bad(format!(
                "armillary_plugin_protocol = {} but this armillary build supports {}",
                self.armillary_plugin_protocol, PROTOCOL_VERSION
            )));
        }
...
"#;
    ...
    fn rejects_bad_protocol_version() {
        let bad = GOOD.replace(
            "armillary_plugin_protocol = 1",
            "armillary_plugin_protocol = 99",
        );
        let err = Manifest::from_str(&bad, &p()).unwrap_err();
        assert!(format!("{err}").contains("armillary_plugin_protocol"));
    }
