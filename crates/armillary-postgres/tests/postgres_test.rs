...
//! `ARMILLARY_TEST_PG_URL` to a connection string, or they will default
...
use armillary_datafusion::run::{NodeRunStats, RunStatus};
use armillary_datafusion::storage::{EnvironmentStorage, IncrementalStateStorage, RunStorage};
use armillary_engine::NodeId;
use armillary_engine::pipeline::Pipeline;
use armillary_engine::storage::PipelineStorage;
use armillary_postgres::{PostgresEnvironmentStore, PostgresPipelineStore, PostgresRunStore};
...
async fn setup_pool() -> Option<(Pool, String)> {
    ...
    // Run schema creation.
    armillary_postgres::ensure_schema(&pool).await.ok()?;
...
async fn pipeline_name_conflict() {
    ...
    assert!(
        matches!(
            err,
            armillary_engine::pipeline_store::PipelineStoreError::NameConflict(_)
        ),
...
async fn environment_create_and_delete() {
    ...
    assert!(
        matches!(
            err,
            armillary_datafusion::error::EnvironmentError::AlreadyExists(_)
        ),
    ...
    assert!(matches!(
        err,
        armillary_datafusion::error::EnvironmentError::CannotDeleteProd
    ));
...
async fn environment_update_fallback() {
    ...
    assert!(matches!(
        err,
        armillary_datafusion::error::EnvironmentError::CyclicFallback
    ));
    ...
    assert!(matches!(
        err,
        armillary_datafusion::error::EnvironmentError::ProdCannotHaveFallback
    ));
