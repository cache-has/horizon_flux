...
//! PostgreSQL storage backends for Armillary metadata.
//!
...
mod tests {
    ...
    ) -> &(dyn armillary_engine::PipelineStorage + Send + Sync) {
        s
    }
    ...
    fn assert_run_storage_impl(
        s: &PostgresRunStore,
    ) -> &(dyn armillary_datafusion::RunStorage + Send + Sync) {
        s
    }
    ...
    ) -> &(dyn armillary_datafusion::EnvironmentStorage + Send + Sync) {
        s
    }
