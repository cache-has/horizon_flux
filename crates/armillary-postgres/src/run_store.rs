...
use crate::block_on;
use armillary_datafusion::error::{
    ColumnLineageStoreError, IncrementalStateError, LineageStoreError, RunStoreError,
};
use armillary_datafusion::incremental_state::{IncrementalSchemaRecord, IncrementalState};
use armillary_datafusion::run::{NodeRunStats, PipelineRun, RunId, RunStatus, TestResultSummary};
use armillary_datafusion::storage::{
...
use armillary_engine::column_lineage::{Confidence, RelationshipKind};
use armillary_engine::lineage::{BindingDirection, ResourceFingerprint};
...
impl RunStorage for PostgresRunStore {
    ...
    fn save_failure_report(
        &self,
        report: &armillary_datafusion::failure_report::FailureReport,
    ) -> Result<(), RunStoreError> {
        block_on(async {
            let client = crate::retry::get_client(&self.pool).await.map_err(pg_err)?;
    ...
    fn get_failure_report(
        ...
        run_id: &RunId,
        node_id: &str,
    ) -> Result<Option<armillary_datafusion::failure_report::FailureReport>, RunStoreError> {
        block_on(async {
            let client = crate::retry::get_client(&self.pool).await.map_err(pg_err)?;
            let rows = client
                .query(
            ...
            match rows.first() {
                Some(row) => {
                    ...
                    let report: armillary_datafusion::failure_report::FailureReport =
...
fn pg_row_to_stored_column_edge(row: &tokio_postgres::Row) -> StoredColumnEdge {
    use armillary_engine::column_lineage::ColumnEdge;
...
impl armillary_datafusion::SlaStorage for PostgresRunStore {
    fn save_evaluations(
        &self,
        evaluations: &[armillary_engine::SlaEvaluation],
    ) -> Result<(), armillary_datafusion::error::RunStoreError> {
        block_on(async {
            let client = crate::retry::get_client(&self.pool).await.map_err(pg_err)?;
    ...
    fn latest_evaluations(
        &self,
    ) -> Result<Vec<armillary_engine::SlaEvaluation>, armillary_datafusion::error::RunStoreError>
    {
        block_on(async {
            let client = crate::retry::get_client(&self.pool).await.map_err(pg_err)?;
            let rows = client
    ...
    fn latest_evaluation(
        &self,
        fingerprint: &str,
    ) -> Result<Option<armillary_engine::SlaEvaluation>, armillary_datafusion::error::RunStoreError>
    {
        block_on(async {
            let client = crate::retry::get_client(&self.pool).await.map_err(pg_err)?;
            let row = client
...
fn pg_row_to_sla_evaluation(row: &tokio_postgres::Row) -> armillary_engine::SlaEvaluation {
    let status_str: String = row.get(2);
    let status = match status_str.as_str() {
        "ok" => armillary_engine::SlaStatus::Ok,
        "warning" => armillary_engine::SlaStatus::Warning,
        "breach" => armillary_engine::SlaStatus::Breach,
        _ => armillary_engine::SlaStatus::Unknown,
    };
    armillary_engine::SlaEvaluation {
