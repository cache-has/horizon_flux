...
//! timestamps, and counters — used by `armillary metadata export` to copy data
//! from another backend without generating new identifiers.

use armillary_datafusion::environment::{Environment, TableOverride};
use armillary_datafusion::run::{NodeRunStats, PipelineRun};
use armillary_engine::pipeline_store::{PipelineRecord, PipelineVersion};
...
async fn insert_node_stats(
    ...
    run_id: &armillary_datafusion::run::RunId,
    stats: &NodeRunStats,
) -> Result<(), String> {
    client
        .execute(
