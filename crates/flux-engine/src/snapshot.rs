...
//! produced here and emit their own SQL / file rewrites; flux-engine itself
...
pub const FLUX_VALID_FROM: &str = "flux_valid_from";

/// Column added to a snapshot target marking when this version stopped being
/// current. `NULL` means "still current".
pub const FLUX_VALID_TO: &str = "flux_valid_to";

/// Boolean convenience column equivalent to `flux_valid_to IS NULL`.
pub const FLUX_IS_CURRENT: &str = "flux_is_current";

/// Stable surrogate key for a single historical version of a row.
pub const FLUX_SCD_ID: &str = "flux_scd_id";
...
pub struct ScdMetadataColumn {
    ...
    /// `false` for `flux_valid_to` (NULL = still current); `true` everywhere else.
    pub not_null: bool,
}

/// The canonical set of metadata columns flux adds to a snapshot target on
...
///
/// The result is what flux compares between the target's current version
...
/// useful for the `flux snapshot diff` CLI subcommand.
...
/// One row currently marked `flux_is_current = true` in the target. The
