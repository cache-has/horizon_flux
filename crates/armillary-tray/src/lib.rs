...
//! System tray icon and desktop notifications for Armillary.
//!
...
use armillary_datafusion::{ExecutionEvent, RunStatus};
...
impl TrayHandle {
    /// Tell the tray to open a specific URL when "Open Armillary" is clicked.
...
fn handle_execution_event(
    ...
    match event {
        ExecutionEvent::RunStarted { pipeline_name, .. } => {
            ...
            let tooltip = format!("Armillary — running {pipeline_name}");
        ...
        ExecutionEvent::RunCompleted {
            ...
            match status {
                RunStatus::Success => {
                    ...
                    let _ = tray.set_tooltip(Some("Armillary"));
                ...
                RunStatus::Failed => {
                    ...
                    let _ = tray.set_tooltip(Some("Armillary — last run failed"));
                    if notifications_enabled {
        ...
        ExecutionEvent::NodeFailed {
            ...
            let _ = tray.set_icon(Some(icon::error_icon()));
            let tooltip = format!("Armillary — node {node_id} failed");
            let _ = tray.set_tooltip(Some(&tooltip));
            if notifications_enabled {
