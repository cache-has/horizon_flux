...
impl SecretStore {
    /// Default store directory: `~/.armillary/`.
    pub fn default_dir() -> Option<PathBuf> {
        dirs::home_dir().map(|h| h.join(".armillary"))
    }

    /// Default store database path: `~/.armillary/secrets.db`.
    ...
    pub fn init(path: &Path, password: &[u8]) -> Result<Self, SecretError> {
        ...
        let verification = crypto::encrypt(&key, b"horizon-armillary-secrets")?;
    ...
    pub fn open(path: &Path, password: &[u8]) -> Result<Self, SecretError> {
        ...
        if plaintext != b"horizon-armillary-secrets" {
