...
//! `armillary lineage` subcommands — cross-pipeline lineage graph queries
...
fn build_column_graph(
    store: &dyn armillary_datafusion::ColumnLineageStorage,
    environment: &str,
    ...
    let mut by_pipeline: HashMap<PipelineId, Vec<armillary_engine::ColumnEdge>> = HashMap::new();
...
fn column_impact(
    ...
    let mut by_pipeline: HashMap<PipelineId, Vec<&armillary_engine::TraceEdge>> = HashMap::new();
    for e in &all_edges {
        by_pipeline
