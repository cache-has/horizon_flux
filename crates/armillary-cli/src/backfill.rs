...
use armillary_datafusion::BackfillStorage;
...
fn start(
    ...
    if !matches!(range_definition, RangeDefinition::Sql { .. }) {
        let expanded = armillary_engine::backfill::expand_range(&range_definition)
    ...
    let base_options = armillary_datafusion::ExecutionOptions {
        environment: env.to_string(),
    ...
    let opts = armillary_datafusion::BackfillRunOptions {
    ...
    let result = rt.block_on(armillary_datafusion::backfill::start_backfill(
        backfill, opts,
    ));
...
fn resume(backfill_id: &str, format: OutputFormat, metadata_url: Option<&str>) -> Result<()> {
    ...
    let result = rt.block_on(armillary_datafusion::backfill::resume_backfill(
        &bf_id, opts,
    ));
    let _ = rt.block_on(progress_handle);
    rt.shutdown_background();
...
fn cancel(backfill_id: &str, format: OutputFormat, metadata_url: Option<&str>) -> Result<()> {
    ...
    armillary_datafusion::backfill::cancel_backfill(&bf_id, &*store)
...
fn print_backfill_event(event: &armillary_datafusion::BackfillEvent) {
    use armillary_datafusion::BackfillEvent;
    match event {
