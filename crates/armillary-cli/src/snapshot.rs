...
//! `armillary snapshot` subcommands — `diff` and `history` for SCD2 snapshot
...
use armillary_connectors::config::PostgreSqlConfig;
use armillary_datafusion::SecretResolver;
use armillary_engine::PipelineRecord;
use armillary_engine::materialization::{ChangeDetection, SnapshotPolicy, WriteStrategy};
use armillary_engine::node::{NodeKind, SinkConfig};
use armillary_engine::snapshot::{FLUX_IS_CURRENT, FLUX_SCD_ID, FLUX_VALID_FROM, FLUX_VALID_TO};
...
pub enum SnapshotAction {
    /// Show what `armillary run` *would* change on a snapshot sink without
...
fn resolve_snapshot_sink(
    ...
    let node = record
        .pipeline
        .node(&armillary_engine::NodeId::new(node_id))
    ...
    if sink_cfg.connector != "postgresql" {
        anyhow::bail!(
            "`armillary snapshot` v1 supports the `postgresql` sink only — node `{node_id}` uses \
    ...
    // connection string we end up with matches what `armillary run` would use.
    ...
    let resolved_vars = armillary_engine::variables::ResolvedVariables::resolve(
        &record.pipeline,
        variable_overrides,
        &armillary_engine::variables::BuiltinContext {
...
/// Mirror of `armillary_datafusion::executor::merge_override` (not pub there).
...
fn diff_cmd(
    ...
    let sink_node_id = armillary_engine::NodeId::new(node_id);
...
async fn run_upstream_dry_run(
    ...
    variable_overrides: HashMap<String, serde_json::Value>,
    secret_resolver: Option<Arc<dyn SecretResolver>>,
) -> Result<armillary_datafusion::PipelineResult> {
    ...
    let (pipeline_result, _run) = armillary_datafusion::PipelineExecutor::execute(
        &record.pipeline,
        &provider_registry,
        &options,
    )
...
/// target where `armillary_is_current = true`. The signal is computed inside
        ...
        OutputFormat::Json => {
            let json_rows: Vec<serde_json::Value> = rows
                ...
                .map(|r| {
                    ...
                    serde_json::json!({
                        "armillary_scd_id": r.scd_id,
                        "armillary_valid_from": r.valid_from,
                        "armillary_valid_to": r.valid_to,
                        "armillary_is_current": r.is_current,
