...
//! `armillary incremental` subcommands — reset/status/list/plan for incremental
...
fn plan(
    ...
    // the same shape `armillary run` would.
    if let Err(errors) = armillary_engine::dag::validate(&record.pipeline) {
        let msgs: Vec<String> = errors.iter().map(|e| e.to_string()).collect();
        anyhow::bail!("DAG validation failed:\n{}", msgs.join("\n"));
    }

    let state_store: Arc<dyn armillary_datafusion::IncrementalStateStorage> =
        Arc::clone(&stores.incremental_state_store);

    let plans = armillary_datafusion::incremental_coordinator::build_plans(
        &record.pipeline,
...
fn effective_env(env: Option<&str>, record: &armillary_engine::PipelineRecord) -> String {
...
fn print_state_table(states: &[armillary_datafusion::IncrementalState]) {
    println!(
        "{}",
