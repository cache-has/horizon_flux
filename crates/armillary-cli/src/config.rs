...
//! 2. `ARMILLARY_METADATA_URL` environment variable
//! 3. `~/.armillary/config.toml` `[metadata]` section
...
const METADATA_URL_ENV: &str = "ARMILLARY_METADATA_URL";

/// The active metadata backend.
#[derive(Debug, Clone)]
pub enum MetadataBackend {
    /// Local SQLite files in `~/.armillary/`.
    Sqlite,
    /// Remote PostgreSQL database.
    Postgresql { connection_string: String },
}

/// Top-level config file structure (`~/.armillary/config.toml`).
#[derive(Debug, Deserialize, Default)]
struct ConfigFile {
    #[serde(default)]
    metadata: Option<MetadataSection>,
    #[serde(default)]
    logging: Option<armillary_observability::config::LoggingConfig>,
    #[serde(default)]
    metrics: Option<armillary_observability::config::MetricsConfig>,
    #[serde(default)]
    tracing: Option<armillary_observability::config::TracingConfig>,
    #[serde(default)]
    openlineage: Option<armillary_observability::config::OpenLineageConfig>,
}
...
impl MetadataBackend {
    ...
    /// Human-readable description for `armillary config show`.
    pub fn display_source(&self, cli_url: Option<&str>, data_dir: &Path) -> &'static str {
        ...
        {
            return "ARMILLARY_METADATA_URL env var";
        }
        let config_path = data_dir.join("config.toml");
        if config_path.exists() {
            return "config.toml";
...
pub fn resolve_logging_config(
    data_dir: &Path,
) -> Option<armillary_observability::config::LoggingConfig> {
    let config_path = data_dir.join("config.toml");
    if !config_path.exists() {
        return None;
    }
...
pub fn resolve_metrics_config(
    data_dir: &Path,
) -> Option<armillary_observability::config::MetricsConfig> {
    let config_path = data_dir.join("config.toml");
    if !config_path.exists() {
        return None;
    }
...
pub fn resolve_tracing_config(
    data_dir: &Path,
) -> Option<armillary_observability::config::TracingConfig> {
    let config_path = data_dir.join("config.toml");
    if !config_path.exists() {
        return None;
    }
...
pub fn resolve_openlineage_config(
    data_dir: &Path,
) -> Option<armillary_observability::config::OpenLineageConfig> {
    let config_path = data_dir.join("config.toml");
    if !config_path.exists() {
        return None;
    }
...
/// Return the data directory (`~/.armillary/`), creating it if needed.
...
pub struct MetadataStores {
    pub pipeline_store: Arc<dyn armillary_engine::PipelineStorage>,
    pub run_store: Arc<dyn armillary_datafusion::RunStorage>,
    pub environment_store: Arc<dyn armillary_datafusion::EnvironmentStorage>,
    pub incremental_state_store: Arc<dyn armillary_datafusion::IncrementalStateStorage>,
    pub lineage_store: Arc<dyn armillary_datafusion::LineageStorage>,
    pub trigger_store: Arc<dyn armillary_scheduler::TriggerStorage>,
    pub backfill_store: Arc<dyn armillary_datafusion::BackfillStorage>,
    pub column_lineage_store: Option<Arc<dyn armillary_datafusion::ColumnLineageStorage>>,
    pub sla_store: Option<Arc<dyn armillary_datafusion::SlaStorage>>,
}
...
fn open_sqlite_stores(data_dir: &Path) -> Result<MetadataStores> {
    ...
    let pipeline_store: Arc<dyn armillary_engine::PipelineStorage> = Arc::new(
        armillary_engine::SqlitePipelineStore::open(&data_dir.join("pipelines.db"), &pipelines_dir)
    ...
    let run_store_concrete = Arc::new(
        armillary_datafusion::SqliteRunStore::open(&data_dir.join("runs.db"))
            .context("failed to open run store")?,
    );
    let run_store: Arc<dyn armillary_datafusion::RunStorage> = run_store_concrete.clone();
    let incremental_state_store: Arc<dyn armillary_datafusion::IncrementalStateStorage> =
        run_store_concrete.clone();
    let lineage_store: Arc<dyn armillary_datafusion::LineageStorage> = run_store_concrete.clone();
    let sla_store: Option<Arc<dyn armillary_datafusion::SlaStorage>> =
        Some(run_store_concrete.clone());
    let column_lineage_store: Option<Arc<dyn armillary_datafusion::ColumnLineageStorage>> =
        Some(run_store_concrete);
    let environment_store: Arc<dyn armillary_datafusion::EnvironmentStorage> = Arc::new(
        armillary_datafusion::SqliteEnvironmentStore::open(&data_dir.join("environments.db"))
            .context("failed to open environment store")?,
    );
    let trigger_store: Arc<dyn armillary_scheduler::TriggerStorage> = Arc::new(
        armillary_scheduler::SqliteTriggerStore::open(&data_dir.join("triggers.db"))
            .context("failed to open trigger store")?,
    );
    let backfill_store: Arc<dyn armillary_datafusion::BackfillStorage> = Arc::new(
        armillary_datafusion::SqliteBackfillStore::open(&data_dir.join("backfills.db"))
...
fn open_postgres_stores(connection_string: &str) -> Result<MetadataStores> {
    let pool = armillary_postgres::create_pool(connection_string)
    ...
    let ensure = async {
        armillary_postgres::ensure_schema(&pool)
            .await
            .map_err(|e| anyhow::anyhow!("{e}"))
    };
    ...
    let pipeline_store: Arc<dyn armillary_engine::PipelineStorage> =
        Arc::new(armillary_postgres::PostgresPipelineStore::new(pool.clone()));
    let run_store_concrete = Arc::new(armillary_postgres::PostgresRunStore::new(pool.clone()));
    let run_store: Arc<dyn armillary_datafusion::RunStorage> = run_store_concrete.clone();
    let incremental_state_store: Arc<dyn armillary_datafusion::IncrementalStateStorage> =
        run_store_concrete.clone();
    let lineage_store: Arc<dyn armillary_datafusion::LineageStorage> = run_store_concrete.clone();
    ...
    let column_lineage_store: Option<Arc<dyn armillary_datafusion::ColumnLineageStorage>> =
        Some(run_store_concrete);
    let environment_store: Arc<dyn armillary_datafusion::EnvironmentStorage> = Arc::new(
        armillary_postgres::PostgresEnvironmentStore::new(pool)
            .map_err(|e| anyhow::anyhow!("{e}"))
