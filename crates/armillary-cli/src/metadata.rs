...
//! CLI handlers for `armillary metadata` subcommands.
//!
...
fn handle_init(url: &str, format: OutputFormat) -> Result<()> {
    let pool = armillary_postgres::create_pool(url)
...
fn handle_export(to_url: &str, format: OutputFormat, metadata_url: Option<&str>) -> Result<()> {
    ...
    let target_pool = armillary_postgres::create_pool(to_url)
        .map_err(|e| anyhow::anyhow!("{e}"))
    ...
    rt.block_on(async {
        armillary_postgres::ensure_schema(&target_pool)
            .await
            .map_err(|e| anyhow::anyhow!("{e}"))
    })
    ...
    for env in &environments {
        rt.block_on(armillary_postgres::bulk::insert_environment(
            &target_pool,
            env,
        ))
        .map_err(|e| anyhow::anyhow!("{e}"))
        ...
        for ovr in &overrides {
            rt.block_on(armillary_postgres::bulk::insert_table_override(
                &target_pool,
                ovr,
            ))
            .map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    for record in &pipelines {
        rt.block_on(armillary_postgres::bulk::insert_pipeline(
            &target_pool,
            record,
        ))
        .map_err(|e| anyhow::anyhow!("{e}"))
        ...
        for version in &versions {
            rt.block_on(armillary_postgres::bulk::insert_pipeline_version(
                &target_pool,
                version,
            ))
            .map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    for run in &runs {
        rt.block_on(armillary_postgres::bulk::insert_run(&target_pool, run))
            .map_err(|e| anyhow::anyhow!("{e}"))
...
fn handle_import(from_url: &str, format: OutputFormat, metadata_url: Option<&str>) -> Result<()> {
    ...
    let source_pool = armillary_postgres::create_pool(from_url)
        .map_err(|e| anyhow::anyhow!("{e}"))
    ...
    let source_pipeline_store = armillary_postgres::PostgresPipelineStore::new(source_pool.clone());
    let source_run_store = armillary_postgres::PostgresRunStore::new(source_pool.clone());
    let source_env_store = armillary_postgres::PostgresEnvironmentStore::new(source_pool)
        .map_err(|e| anyhow::anyhow!("{e}"))?;
...
fn import_to_sqlite(
    ...
    source_pipelines: &dyn armillary_engine::PipelineStorage,
    source_runs: &dyn armillary_datafusion::RunStorage,
    source_envs: &dyn armillary_datafusion::EnvironmentStorage,
    format: OutputFormat,
) -> Result<()> {
    ...
    let target_run_store = armillary_datafusion::SqliteRunStore::open(&data_dir.join("runs.db"))
...
fn import_via_traits(
    ...
    data_dir: &std::path::Path,
    source_pipelines: &dyn armillary_engine::PipelineStorage,
    _source_runs: &dyn armillary_datafusion::RunStorage,
    source_envs: &dyn armillary_datafusion::EnvironmentStorage,
    format: OutputFormat,
) -> Result<()> {
