...
use chrono::Utc;
use flux_scheduler::{SchedulerError, Trigger, TriggerId, TriggerKind, TriggerState};
...
struct CreateTriggerRequest {
    ...
    kind: TriggerKind,
    #[serde(default)]
    run_policy: flux_scheduler::RunPolicy,
    #[serde(default)]
...
struct UpdateTriggerRequest {
    ...
    run_policy: Option<flux_scheduler::RunPolicy>,
    variable_overrides: Option<HashMap<String, serde_json::Value>>,
...
async fn fire_trigger(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    ...
    match scheduler.manual_fire(&tid) {
        ...
        Err(SchedulerError::Store(flux_scheduler::TriggerStoreError::NotFound(_))) => {
            ApiError::not_found("trigger", &id).into_response()
        }
        Err(e) => ApiError::internal(e.to_string()).into_response(),
    }
...
fn compute_next_fire(kind: &TriggerKind) -> Option<String> {
    match kind {
        TriggerKind::Cron {
            expression,
            timezone,
        } => flux_scheduler::cron::CronSchedule::parse(expression, timezone)
            .ok()
        ...
        TriggerKind::Interval { every, start_at } => {
            ...
            flux_scheduler::interval::Iso8601Duration::parse(every)
                .ok()
