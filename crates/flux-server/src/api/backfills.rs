...
use chrono::Utc;
use flux_datafusion::{BackfillEvent, BackfillRunOptions, ExecutionEvent, ExecutionOptions};
...
struct BackfillDetailResponse {
    ...
    iterations: Vec<flux_engine::backfill::BackfillIteration>,
}
...
async fn create_backfill(
    ...
    let pipeline_id = match req.pipeline_id.parse::<flux_engine::PipelineId>() {
        Ok(id) => id,
    ...
    if !matches!(req.range_definition, RangeDefinition::Sql { .. }) {
        if let Err(e) = flux_engine::backfill::expand_range(&req.range_definition) {
    ...
    // HTTP response.
    tokio::spawn(async move {
        if let Err(e) = flux_datafusion::backfill::start_backfill(backfill, opts).await {
...
async fn resume_backfill(
    ...
    tokio::spawn(async move {
        if let Err(e) = flux_datafusion::backfill::resume_backfill(&bf_id_clone, opts).await {
...
async fn cancel_backfill(
    ...
    if let Err(e) =
        flux_datafusion::backfill::cancel_backfill(&bf_id, state.backfill_store.as_ref())
    {
