...
//! Wire protocol for the Armillary plugin system — shared between
//! `armillary-plugin-host` (host side) and `armillary-plugin-sdk` (plugin side) so the
...
//! Write}`, and no dependency on any other armillary crate. Plugin authors should
//! be able to depend on it directly without pulling in armillary internals.
...
/// The plugin protocol major version this build of armillary speaks.
