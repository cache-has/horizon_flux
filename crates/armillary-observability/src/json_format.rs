...
//! Custom JSON logging layer producing the documented armillary event envelope.
//!
...
//!   "level": "info",
//!   "component": "armillary_datafusion::executor",
...
/// A tracing layer that writes events as the documented armillary JSON envelope.
    ...
    W: Write + Send + 'static,
{
    ...
    fn on_event(&self, event: &tracing::Event<'_>, ctx: Context<'_, S>) {
        ...
        // Check if this is a structured armillary event.
        if let Some(armillary_event_json) = visitor.fields.remove("armillary_event") {
            // Parse the serialized FluxEvent and merge into envelope.
            if let Ok(Value::Object(obj)) = serde_json::from_str::<Value>(&armillary_event_json) {
                for (k, v) in obj {
                    envelope.insert(k, v);
                }
            }
            // Remove the internal marker field.
            visitor.fields.remove("armillary_event_type");
