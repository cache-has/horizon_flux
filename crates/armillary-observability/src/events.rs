...
//! Stable, versioned armillary event types with structured payloads.
//!
...
/// Emit a structured armillary event as a tracing event.
///
/// This produces a `tracing::info!` event with the `armillary_event` field set to
...
macro_rules! emit_event {
    ($event:expr) => {
        ...
        tracing::info!(
            armillary_event = %serialized,
            armillary_event_type = $event.event_type(),
            "{}",
            $event.event_type()
        );
    };
...
/// All documented armillary event types.
///
...
pub struct IncrementalStateAdvanced {
    ...
    pub run_id: String,
    pub node_id: String,
    /// Serialized `MaterializationReceipt` from armillary-datafusion.
