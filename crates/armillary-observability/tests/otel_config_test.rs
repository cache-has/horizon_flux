...
use armillary_observability::config::TracingConfig;
...
fn tracing_config_defaults() {
    ...
    assert_eq!(config.service_name, "armillary");
...
"#;
    ...
    assert_eq!(config.service_name, "my-armillary");
...
fn current_traceparent_returns_none_without_otel() {
    ...
    let tp = armillary_observability::otel::current_traceparent();
    assert!(tp.is_none());
}
...
fn build_provider_disabled_returns_none() {
    ...
    let result = armillary_observability::otel::build_provider(&config);
